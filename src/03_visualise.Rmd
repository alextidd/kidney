...
# module load R/4.4.0 ; Rscript -e "rmarkdown::render('src/03_visualise.Rmd', output_dir = 'out/nanoseq/analysis/')" # nolint: line_length_linter.
...
The diploid human genome size for GRCh37 is 6,274,289,386 bp, so we multiply the
burdens by the full genome size to get mutations per genome. We then divide by
donor age. to get mutations per genome per year.

```{r get_burdens}
diploid_bp <- 6274289386
# get all burdens
burdens <-
  post[["mut_burden"]] %>%
  ...
  dplyr::mutate(indel_countgenome = indel_burden * diploid_bp,
                indel_countgenomeyearly = indel_countgenome / donor_age,
                snv_countgenome = snv_burden * diploid_bp,
