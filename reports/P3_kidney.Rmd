...
[NanoSeq pipeline](https://github.com/cancerit/NanoSeq) was run on all `BAM`s
...
however, was raised from 0.01 to 0.05. This parameter assigns the maximum normal
variant allele frequency (VAF) (???). Because the pseudobulked matched normal
shares some of its reads with each normal cut, we must relax this to avoid
filtering out somatic variants inevitably found in both (???).
